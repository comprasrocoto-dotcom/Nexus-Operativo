              ...
              return (
                <li key={m.clave} className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-700">{m.emoji ? m.emoji + " " : ""}{m.etiqueta}</span>
                  </div>
                  <div className="flex items-center gap-2">
...
/**
 * Pestana Configuracion. Configuracion propia de la sede.
 * Se apoya en Op_Modulos (registro data-driven) para activar/desactivar modulos por sede.
 */
export function Configuracion({ sedeId }: ConfiguracionProps) {
    ...
    <div className="space-y-4">
      ...
      <Card className="p-5 rounded-2xl shadow-soft">
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-slate-400 text-sm">No hay modulos registrados en Op_Modulos.</div>
        ) : (
          <ul className="space-y-2">
            {modulos.map((m) => {
              const activo = m.Estado === "Activo";
              return (
                <li key={m.ID} className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-700">{m.Icono} {m.Nombre}</span>
                  </div>
                  <div className="flex items-center gap-2">
